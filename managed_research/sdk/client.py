...
from managed_research.auth import (
    BACKEND_URL_BASE,
    encrypt_for_backend,
    get_api_key,
...
class SmrControlClient:
    ...
    def github_org_status_typed(self) -> SmrIntegrationStatus:
        return SmrIntegrationStatus.from_dict(
            _coerce_dict(self.github_org_status(), label="github_org_status")
        )
    ...
    def github_org_oauth_start_typed(self, *, redirect_uri: str | None = None) -> SmrOAuthStart:
        return SmrOAuthStart.from_dict(
            _coerce_dict(
                self.github_org_oauth_start(redirect_uri=redirect_uri),
                label="github_org_oauth_start",
            )
        )
    ...
    def linear_status_typed(self, project_id: str) -> SmrIntegrationStatus:
        return SmrIntegrationStatus.from_dict(
            _coerce_dict(self.linear_status(project_id), label="linear_status")
        )
    ...
    def linear_list_teams_typed(self, project_id: str) -> SmrLinearTeamListing:
        payload = self.linear_list_teams(project_id)
        if isinstance(payload, list):
            return SmrLinearTeamListing.from_payload(payload)
        return SmrLinearTeamListing.from_payload(
            _coerce_dict(payload, label="linear_list_teams")
        )
    ...
    def chatgpt_connection_status_typed(
        ...
        return SmrIntegrationStatus.from_dict(
            _coerce_dict(
                self.chatgpt_connection_status(project_id=project_id),
                label="chatgpt_connection_status",
            )
        )
    ...
    def provider_key_status_typed(
        ...
        return SmrProviderKeyStatus.from_dict(
            _coerce_dict(
                self.provider_key_status(project_id, provider, funding_source),
                label="provider_key_status",
            )
        )
